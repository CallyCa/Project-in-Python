...
class Pessoa(object):
    def __init__(self, user = '', password = '', nome = ''):
        self.user = user
...
class ArquivoDbm(object):
    ...
    def insere_usuario(self, user = '', password = '', nome = ''):
        """insere um novo usuário com sua senha, nome e e-mail no arquivo dbm"""
        try:
            with shelve.open(self.arquivo) as p_db:
                p_db[user] = [password, nome]
        except :
            print('Ocorreu um erro inesperado...')
            return ERRO
...
class Login(object):
    ...
    def declara_widgets_signin(self, instancia):
        ...
        self.password_signin = Label(self.frame3, text = 'Ch4vB Cr3pt0', bg = COR_DE_FUNDO, font = self.fonte, fg = 'black')
        ...
        self.create_signin = Button(self.subframe0, text = 'REGISTRO', width = 7, bg = self.enter_signin['bg'], command = self.create_user)
    ...
    def declara_widgets_createuser(self, instancia):
        ...
        #Texto de que pede a senha
        self.password_createuser = Label(self.frame16, text = 'Ch4vB Cr3pt0', bg = COR_DE_FUNDO, font = self.fonte, fg = 'green')
    ...
    def __inserir(self):
        ...
        if (self.pessoa.nome or self.pessoa.user or self.pessoa.__password) == '' :
            ...
            if self.arq_dbm.verifica_usuario(self.pessoa.user) == USUARIO_NAO_CADASTRADO:
                self.arq_dbm.insere_usuario(self.pessoa.user, self.pessoa.__password, self.pessoa.nome)
